...
export default function ChatPage() {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // สมมติว่าข้อมูลการวิเคราะห์อยู่ใน data.result หรือ data.analysis
  ...
  return (
    <div className="h-screen flex flex-col bg-gradient-to-b from-[#0A0F1C] to-[#1A1F2C] text-white overflow-hidden">
      ...
      />
      <div 
        className="fixed top-20 right-20 w-40 h-40 bg-indigo-600/10 rounded-full blur-[100px] animate-pulse pointer-events-none hidden sm:block" 
        aria-hidden="true" 
        role="presentation"
      />
      <div 
        className="fixed bottom-40 left-20 w-56 h-56 bg-cyan-500/10 rounded-full blur-[120px] animate-pulse pointer-events-none hidden md:block" 
        aria-hidden="true"
        role="presentation"
      />

      {/* Header */}
      <header className="p-4 text-center bg-black/20 font-semibold text-lg backdrop-blur-sm" role="banner">
      ...
      <main className="flex-1 flex flex-col lg:flex-row overflow-hidden" role="main" aria-live="polite">
        ...
        >
          <div className="absolute inset-0">
            ...
            {/* เพิ่ม alt text เพื่อบรรยายภาพหากจำเป็น */}
            <img
              src={currentPlayingId ? "/img/video-unscreen.gif" : "/img/video-unscreen-still.jpg"}
              alt="AI Avatar animation"
              className="w-full h-full object-cover object-center"
            />
            <div className="absolute inset-0 bg-gradient-to-r from-[#0A0F1C]/50 via-transparent to-[#0A0F1C]/80" aria-hidden="true" role="presentation" />
            <div className="absolute inset-0 bg-gradient-to-t from-[#0A0F1C]/40 to-transparent" aria-hidden="true" role="presentation" />
            <div className="absolute inset-0 bg-gradient-to-b from-[#0A0F1C]/40 to-transparent" aria-hidden="true" role="presentation" />
          </div>
        </section>
                  ...
                  {message.role === 'assistant' && !message.error && (
                    <div className="flex gap-2 ml-2">
                      ...
                      {uploadedFile && (
                        <button 
                          className="p-2 bg-cyan-500/20 hover:bg-cyan-500/30 rounded-full transition-colors" 
                          aria-label={`Open PDF ${uploadedFile.name}`}
                        >
                          <FaRegFilePdf className="w-4 h-4 md:w-5 md:h-5 text-cyan-400" aria-hidden="true" />
                        </button>
                      )}
                    </div>
                  )}
